...
def StarchColor(st):
    if st.invisible == True:
        health = st.max_health
        if(health > 50):
            return Back.GREEN
    else:
        health = st.health
...
def printMap(V):
    ...
    for st in s_archers:
        # a= 2*st.position[0]
        # b = 2*st.position[1]
        # map_matrix[a][b] = archColor(st) + Fore.BLACK + 'ST'
        # map_matrix[a+1][b] = archColor(st) + Fore.BLACK + '  '
        # map_matrix[a][b+1] = archColor(st) + Fore.BLACK + ' '
        # map_matrix[a+1][b+1] = archColor(st) + Fore.BLACK + ' '
        if st.invisible == True:
            a= 2*st.position[0]
            b = 2*st.position[1]
            map_matrix[a][b] = archColor(st) + Fore.GREEN + ''
            map_matrix[a+1][b] = archColor(st) + Fore.GREEN + ''
            map_matrix[a][b+1] = archColor(st) + Fore.GREEN + ''
            map_matrix[a+1][b+1] = archColor(st) + Fore.GREEN + ''
        else:
            a= 2*st.position[0]
            b = 2*st.position[1]
            map_matrix[a][b] = archColor(st) + Fore.BLACK + 'ST'
    ...
    for hl in healers:
        ...
        map_matrix[a+1][b] = blColor(hl) + Fore.BLACK + 'AL'
        map_matrix[a][b+1] = blColor(hl) + Fore.BLACK + 'ER'
        map_matrix[a+1][b+1] = blColor(hl) + Fore.BLACK + 'S '
